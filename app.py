...
def normalize_publication_status(value) -> str:
    s = str(value).strip().upper() if pd.notna(value) else ""
    if not s or s in {"NAN", "NONE"}:
        return "SIN STATUS"
    if "ACTIVA" in s or s in {"ACTIVE", "ACTIVO", "PUBLICADA"}:
        return "ACTIVA"
    if any(tok in s for tok in ["PAUS", "INACT", "CLOSE", "DESACT", "FINAL", "DELETE", "ARCHIV", "SUSP"]):
        return "DESACTIVADA"
    return s
...
def build_action_table(master, sales_windows, total_hist, purchase_summary, publications, ads_by_sku):
    ...
    if publications is not None and not publications.empty:
        for sku, grp in publications.groupby("sku", sort=False):
            grp = grp.copy()
            grp["status"] = grp["status"].apply(normalize_publication_status) if "status" in grp.columns else "SIN STATUS"
            pr = choose_primary_publication(grp)
            if pr is not None:
                pub_map[sku] = grp.copy()
                activos = int(grp["status"].astype(str).str.upper().eq("ACTIVA").sum()) if "status" in grp.columns else 0
                total = int(len(grp))
                no_activas = int(total - activos)
                pub_primary_rows.append({
                    ...
                    "publicaciones_total": total,
                    "publicaciones_activas": activos,
                    "publicaciones_no_activas": no_activas,
...
def build_model_cached(master_bytes, ventas_bytes, compras_bytes=None, pubs_bytes=None, ads_bytes=None, keywords_bytes=None):
    if not master_bytes or not ventas_bytes or not pubs_bytes:
        raise ValueError("Faltan archivos base válidos para construir el modelo (maestra, ventas y publicaciones ML).")
...
current_state_sig = combined_sig
try:
    model = build_shared_model(resolved_files)
except Exception as e:
    st.error(f"No pude construir el modelo con los archivos activos: {e}")
    st.stop()
...
with tabs[0]:
    ...
    st.subheader("Brechas maestra vs última compra")
    ...
    cost_limit = int(b2.number_input("Filas costo", min_value=20, max_value=5000, value=200, step=20, key="cost_gap_limit"))
    ...
    brechas_show = brechas[["sku", "descripcion", "costo_maestra", "ultimo_costo_compra", "brecha_costo_pct", "estado_brecha_costo", "accion_sugerida"]].copy()
    ...
    st.dataframe(brechas_show.head(cost_limit), use_container_width=True, hide_index=True, height=280)

    st.subheader("Brecha comercial real: monto en simulación maestra vs ingreso estimado del reporte ML")
    c1, c2 = st.columns([1.2, 1.2])
    only_with_pub = c1.selectbox("Cobertura ML", ["Solo con publicación", "Todos"], key="commercial_pub_filter")
    commercial_limit = int(c2.number_input("Filas comercial", min_value=20, max_value=5000, value=200, step=20, key="commercial_limit"))
    ...
    commercial_required_cols = ["sku", "descripcion", "status_publicacion", "monto_sim", "ingreso_estimado_ml", "brecha_monto_sim_pct", "precio_ml_base", "precio_ml_oferta", "precio_ml_actual", "total_cargo_pct_ml", "costo_fijo_ml", "margen_ml_reportado", "margen_ml_con_ads"]
    ...
    commercial_show.columns = ["SKU", "Descripción", "Status", "Monto simulación", "Ingreso est. ML", "Brecha comercial %", "Precio base ML", "Precio oferta ML", "Precio final ML", "Fee ML %", "Costo fijo ML", "Margen ML reportado", "Margen ML con ads"]
    for c in ["Monto simulación", "Ingreso est. ML", "Precio base ML", "Precio oferta ML", "Precio final ML", "Costo fijo ML"]:
        commercial_show[c] = commercial_show[c].map(fmt_money)
    for c in ["Brecha comercial %", "Fee ML %", "Margen ML reportado", "Margen ML con ads"]:
        commercial_show[c] = commercial_show[c].map(fmt_pct)
    st.dataframe(commercial_show.head(commercial_limit), use_container_width=True, hide_index=True, height=320)
...
with tabs[1]:
    ...
    if row.empty:
        ...
        if ads_row.empty:
            ...
            a1.metric("Ads activo en maestra", "Sí" if bool(row.get("ads_flag")) else "No")
            a2.metric("Inversión", fmt_money(ar["ads_inversion"]))
            a3.metric("Ingresos ads", fmt_money(ar["ads_ingresos"]))
            a4.metric("ACOS", fmt_pct(ar["ads_acos"]))
            a5.metric("Ventas por publicidad", fmt_int(ar["ads_ventas"]))
        ...
        if pr is None:
            ...
            d4.metric("Status", normalize_publication_status(pr.get('status', '')))
            st.caption(f"Entrega: {pr['entrega']}")

        st.markdown("### Historial de ventas")
        ventas_sku = model["ventas"][model["ventas"]["sku"] == sku].copy()
        if ventas_sku.empty:
            st.info("No encontré ventas para este SKU.")
        else:
            ml_hist = ventas_sku[ventas_sku["canal"] == "ML"].sort_values("fecha", ascending=False).copy()
            tienda_hist = ventas_sku[ventas_sku["canal"] == "TIENDA"].sort_values("fecha", ascending=False).copy()
            v1, v2, v3, v4 = st.columns(4)
            v1.metric("Ventas ML", fmt_int(len(ml_hist)))
            v2.metric("Monto ML", fmt_money(ml_hist["total_linea"].sum() if not ml_hist.empty else np.nan))
            v3.metric("Ventas tienda", fmt_int(len(tienda_hist)))
            v4.metric("Monto tienda", fmt_money(tienda_hist["total_linea"].sum() if not tienda_hist.empty else np.nan))

            h1, h2 = st.columns(2)
            with h1:
                st.markdown("#### Ventas Mercado Libre")
                if ml_hist.empty:
                    st.info("Sin ventas ML.")
                else:
                    ml_show = ml_hist[["fecha", "cantidad", "precio_unitario", "total_linea", "tipo_cliente", "cliente"]].copy()
                    ml_show.columns = ["Fecha", "Cantidad", "Precio Unitario", "Total", "Tipo cliente", "Cliente"]
                    ml_show["Fecha"] = ml_show["Fecha"].map(fmt_date)
                    ml_show["Precio Unitario"] = ml_show["Precio Unitario"].map(fmt_money)
                    ml_show["Total"] = ml_show["Total"].map(fmt_money)
                    st.dataframe(ml_show, use_container_width=True, hide_index=True, height=280)
            with h2:
                st.markdown("#### Ventas Tienda")
                if tienda_hist.empty:
                    st.info("Sin ventas tienda.")
                else:
                    tienda_show = tienda_hist[["fecha", "cantidad", "precio_unitario", "total_linea", "tipo_cliente", "cliente"]].copy()
                    tienda_show.columns = ["Fecha", "Cantidad", "Precio Unitario", "Total", "Tipo cliente", "Cliente"]
                    tienda_show["Fecha"] = tienda_show["Fecha"].map(fmt_date)
                    tienda_show["Precio Unitario"] = tienda_show["Precio Unitario"].map(fmt_money)
                    tienda_show["Total"] = tienda_show["Total"].map(fmt_money)
                    st.dataframe(tienda_show, use_container_width=True, hide_index=True, height=280)
