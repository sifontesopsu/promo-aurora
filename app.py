...
with tabs[2]:
    ...
    sim = ensure_columns(action_table.copy(), {
        "sku": "",
        "descripcion": "",
        "costo_maestra": np.nan,
        "ingreso_estimado_ml": np.nan,
        "precio_ml_actual": np.nan,
        "accion_sugerida": "",
        "total_cargo_pct_ml": np.nan,
        "costo_fijo_ml": np.nan,
        "ads_share_ml_pct": np.nan,
    })
    sim["costo_simulado"] = sim["costo_maestra"] * (1 + proveedor_alza_pct / 100.0)

    # La fuente de verdad para la brecha comercial es:
    # monto en simulación de la maestra vs ingreso estimado del reporte de publicaciones.
    # En repricing mantenemos lógica defensiva para no romper si faltan columnas avanzadas.
    if "ingreso_estimado_ml" in sim.columns and sim["ingreso_estimado_ml"].notna().any():
        sim["ingreso_simulado_ml"] = sim["ingreso_estimado_ml"] * (1 - comision_extra_pct / 100.0)
    else:
        sim["ingreso_simulado_ml"] = np.nan

    neto_obj_factor = 1 - (margen_obj_ml / 100.0)
    sim["precio_sugerido_ml"] = np.where(
        sim["ingreso_simulado_ml"].notna() & (sim["ingreso_simulado_ml"] > 0) & sim["costo_simulado"].notna() & (neto_obj_factor > 0),
    ...
    sim_show = ensure_columns(sim, {
        "sku": "",
        "descripcion": "",
        "costo_maestra": np.nan,
        "costo_simulado": np.nan,
        "precio_ml_actual": np.nan,
        "precio_sugerido_ml": np.nan,
        "delta_precio_sugerido_pct": np.nan,
        "accion_sugerida": "",
    })[[
